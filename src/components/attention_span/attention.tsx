...
import ReasoningIq from '../reasoning_iq/resoning'
...
function AttentionScreen () {
  ...
  const fetchAttentioonSpanScores = async () => {
    await setAttentionSpanScores(await getAttentionSpanIQScores())
  }
  ...
  {attentionSpanScores.length > 0 && (
    <div className="mt-10 ">
          <div className="overflow-y-auto p-4" style={{ height: '50vh' }}>
            {attentionSpanScores.map((item, key) => {
              return (
<div key={key} className="mt-10 relative bg-white p-4 rounded-3xl w-full my-4 shadow-xl">
